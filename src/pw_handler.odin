...
pw_init :: proc() -> linux.Fd {
	ctx.device_inputs = make(map[string]Link)
	ctx.passthrough_nodes = make(map[u32]Node)
	ctx.passthrough_ports = make([dynamic]u32)
	...
	// initialize pipewire
	{
		...
		default_volume, aux_volume := pw_sink_volumes(daemon.state.volume)
		sink_init(
			&ctx.default_sink,
			"mixologist-default",
			default_volume,
	...
	return cast(linux.Fd)pw.loop_get_fd(ctx.loop)
}
...
node_handler :: proc(ctx: ^PwContext, id, version: u32, type: cstring, props: ^pw.spa_dict) {
...
port_handler :: proc(ctx: ^PwContext, id, version: u32, props: ^pw.spa_dict) {
...
link_handler :: proc(ctx: ^PwContext, id, version: u32, props: ^pw.spa_dict) {
...
rebuild_connections :: proc(ctx: ^PwContext) {
	sinks := [?]^Sink{&ctx.default_sink, &ctx.aux_sink}
	for sink in sinks {
...
pw_sink_volumes :: proc(vol: f32) -> (def, aux: f32) {
